...
ser = serial.Serial(
    '/dev/tty.usbmodem14312101',  # Change this lino to the port the device is connected
    baudrate=115200,
    timeout=0.2)
...
while True:
    ...
    net_stat = psutil.net_io_counters()
    bits_sent_per_sec = (
        psutil.net_io_counters().bytes_sent - acc_bytes_sent) * 8
    bits_recv_per_sec = (
        psutil.net_io_counters().bytes_recv - acc_bytes_recv) * 8
